...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if message.chat.type in ["group", "supergroup"]:
        await message.reply(
            get_text_sync("en", "GROUP_START_MESSAGE", bot_username=BOT_USERNAME) or "Hello!",
    ...
    if user.id in _started_users:
        ...
        start_text = get_text_sync(
            language,
            "START_MESSAGE_WITH_IMAGE",
            uid=user.id,
            first_name=escape_html(user.first_name)
        )
        await message.answer(start_text, reply_markup=get_main_menu_keyboard(language))
        return
    ...
    if not user_data:
        ...
        lang_select_msg = get_text_sync("en", "LANG_SELECT_MESSAGE") or "Choose your language:"
...
async def cmd_help(message: Message):
    try:
        ...
        help_text = get_text_sync(language, "HELP_MESSAGE")
...
async def extra_commands_callback(callback: CallbackQuery):
    ...
    try:
        ...
        message_text = get_text_sync(language, "EXTRA_COMMANDS_MESSAGE")
        await safe_edit_message(callback, message_text, get_extra_commands_keyboard(language))
    except Exception as e:
...
async def extra_afk_callback(callback: CallbackQuery):
    ...
    try:
        language = await get_user_language(callback.from_user.id)
        message_text = get_text_sync(language, "AFK_INFO_MESSAGE")
        await safe_edit_message(callback, message_text, get_back_to_extra_keyboard(language))
    except Exception as e:
...
async def extra_quotly_callback(callback: CallbackQuery):
    ...
    try:
        language = await get_user_language(callback.from_user.id)
        message_text = get_text_sync(language, "QUOTLY_INFO_MESSAGE")
        await safe_edit_message(callback, message_text, get_back_to_extra_keyboard(language))
    except Exception as e:
...
async def extra_stickers_callback(callback: CallbackQuery):
    ...
    try:
        language = await get_user_language(callback.from_user.id)
        message_text = get_text_sync(language, "STICKERS_INFO_MESSAGE")
        await safe_edit_message(callback, message_text, get_back_to_extra_keyboard(language))
    except Exception as e:
...
async def extra_memefi_callback(callback: CallbackQuery):
    ...
    try:
        language = await get_user_language(callback.from_user.id)
        message_text = get_text_sync(language, "MEMEFI_INFO_MESSAGE")
        await safe_edit_message(callback, message_text, get_back_to_extra_keyboard(language))
    except Exception as e:
...
async def extra_welcome_callback(callback: CallbackQuery):
    ...
    try:
        language = await get_user_language(callback.from_user.id)
        message_text = get_text_sync(language, "WELCOME_INFO_MESSAGE")
        await safe_edit_message(callback, message_text, get_back_to_extra_keyboard(language))
    except Exception as e:
...
async def extra_tagall_callback(callback: CallbackQuery):
    ...
    try:
        language = await get_user_language(callback.from_user.id)
        message_text = get_text_sync(language, "TAGALL_INFO_MESSAGE")
        await safe_edit_message(callback, message_text, get_back_to_extra_keyboard(language))
    except Exception as e:
...
async def back_to_main_callback(callback: CallbackQuery):
    ...
    try:
        ...
        await safe_edit_message(callback, start_text, get_main_menu_keyboard(language))
    except Exception as e:
