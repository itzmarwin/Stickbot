...
def init_userdb(client: AsyncIOMotorClient, db):
    """
    Initialize userdb with shared client and db instance.
...
async def create_indexes():
    """Create only necessary indexes for user related collections"""
    # Users - sirf user_id zaruri hai, baki queries nahi hoti
    await _db.users.create_index("user_id", unique=True)

    # GBan - sirf user_id zaruri hai
    await _db.gbans.create_index("user_id", unique=True)

    # Served chats - sirf chat_id zaruri hai
    await _db.served_chats.create_index("chat_id", unique=True)

    logger.info("✅ userdb indexes created")
