...
async def send_mention_batch(message_target, mentions: str, header_html: str = None, retry_count: int = 0):
    """Send mention batch — reply mode mein header nahi hota"""
    try:
        ...
        return await send_mention_batch(message_target, mentions, header_html, retry_count + 1)
    except Exception as e:
...
async def send_fresh_message(client: Client, chat_id: int, text: str, retry_count: int = 0):
    """Send fresh message — HTML parse_mode se formatting preserve hogi"""
    try:
...
async def setup_tagall_handlers(client: Client):
    ...
    async def emoji_tagall_command(client: Client, message: Message):
        ...
        try:
            ...
            # KEY FIX: Plain text length check ke liye .split, HTML ke liye .html
            header_html = None
            if not is_reply and has_text:
                plain_text = message.text.split(maxsplit=1)[1]
                if len(plain_text) + (BATCH_SIZE * 50) > 4000:
                    await message.reply_text(
                        f"𝖬𝖾𝗌𝗌𝖺𝗀𝖾 𝗍𝗈𝗈 𝗅𝗈𝗇𝗀!\n\n"
                        f"𝖸𝗈𝗎𝗋 𝗆𝖾𝗌𝗌𝖺𝗀𝖾: {len(plain_text)} 𝖼𝗁𝖺𝗋𝖺𝖼𝗍𝖾𝗋𝗌\n"
                        f"𝖬𝖺𝗑𝗂𝗆𝗎𝗆 𝖺𝗅𝗅𝗈𝗐𝖾𝖽: {MAX_MESSAGE_LENGTH} 𝖼𝗁𝖺𝗋𝖺𝖼𝗍𝖾𝗋𝗌\n\n"
                        f"𝖯𝗅𝖾𝖺𝗌𝖾 𝗌𝗁𝗈𝗋𝗍𝖾𝗇 𝗒𝗈𝗎𝗋 𝗆𝖾𝗌𝗌𝖺𝗀𝖾."
                    )
                    return
                # .html — bold, italic, spoiler, strikethrough, underline, quote sab aayega
                cmd = message.text.split()[0]
                header_html = message.text.html[len(cmd):].strip()
            ...
            if is_reply:
                ...
                for i in range(0, len(members), BATCH_SIZE):
                    ...
                    # header_html mein formatting hai — parse_mode=html se sab aayega
                    text = f"{header_html}\n\n{mentions}" if header_html else mentions
    ...
    async def username_tagall_command(client: Client, message: Message):
        ...
        try:
            ...
            # KEY FIX: .html se formatting preserve hogi
            header_html = None
            if not is_reply and has_text:
                plain_text = message.text.split(maxsplit=1)[1]
                if len(plain_text) + (BATCH_SIZE * 50) > 4000:
