...
def build_mentions(batch, style: str) -> str:
    if style == "username":
        return ", ".join(
            f'<a href="tg://user?id={user.id}">{escape(user.first_name)}</a>'
            for user in batch
        )
    emojis = random.sample(EMOJI_POOL, min(len(batch), len(EMOJI_POOL)))
    return " ".join(
        f'<a href="tg://user?id={user.id}">{emojis[idx]}</a>'
...
async def send_mention_batch(message_target, mentions: str, chat_id: int) -> bool:
    for attempt in range(MAX_RETRIES):
        if not active_tagall.get(chat_id, False):
            return False
        try:
            await message_target.reply_text(mentions, parse_mode=ParseMode.HTML)
            return True
...
async def send_fresh_message(client: Client, chat_id: int, text: str) -> bool:
    for attempt in range(MAX_RETRIES):
        if not active_tagall.get(chat_id, False):
            return False
        try:
...
async def send_tagall_log(client: Client, message: Message, command: str):
    ...
    try:
        ...
        log_text = (
            f"<b>#TagAll Log</b>\n\n"
            f"<b>Group:</b> {escape(chat.title or 'Unknown')}\n"
            f"<b>Group ID:</b> <code>{chat.id}</code>\n"
            f"<b>Admin:</b> <a href=\"tg://user?id={user.id}\">{escape(user.first_name)}</a>\n"
            f"<b>Command:</b> <code>{command}</code>\n"
            f"<b>Invite Link:</b> {invite_link}"
        )
        await client.send_message(LOG_GROUP_ID, log_text, parse_mode=ParseMode.HTML)
    except Exception:
        pass
...
async def run_tagall(
    ...
    try:
        for i in range(0, len(members), batch_size):
            ...
            mentions = build_mentions(batch, style)
...
async def _common_checks(client: Client, message: Message, require_text_or_reply: bool = True, usage_text: str = ""):
    chat_id = message.chat.id
    ...
    if active_tagall.get(chat_id, False):
    ...
    if require_text_or_reply and not is_reply and not has_text:
        await message.reply_text(usage_text)
        return False

    if not is_reply and has_text:
...
async def setup_tagall_handlers(client: Client):
    ...
    async def emoji_tagall_command(client: Client, message: Message):
        chat_id = message.chat.id
        ok = await _common_checks(
            client, message,
            require_text_or_reply=True,
            usage_text=(
        ...
        await run_tagall(
            client, message, members, style="emoji",
            header=message.text.split(maxsplit=1)[1] if has_text else None,
            reply_target=message.reply_to_message if is_reply else None,
        )
    ...
    async def username_tagall_command(client: Client, message: Message):
        ...
        await run_tagall(
            client, message, members, style="username",
            header=message.text.split(maxsplit=1)[1] if has_text else None,
            reply_target=message.reply_to_message if is_reply else None,
        )
