...
async def setup_welcome_handlers(client: Client):
    ...
    async def setwelcome_command(client: Client, message: Message):
        ...
        try:
            ...
            # spoiler, underline, links, premium emoji — sab preserve ho
    ...
    async def welcome_new_member(client: Client, member_update: ChatMemberUpdated):
        try:
            # Check 1: new_chat_member hona chahiye
            ...
            # Check 2: New status MEMBER hona chahiye
            if new_status != ChatMemberStatus.MEMBER:
                return

            # Check 3: Genuine join filter
            # None     → MEMBER = fresh join ✅
            # LEFT     → MEMBER = wapas join kiya ✅
            # BANNED   → MEMBER = unban ❌ skip
            # RESTRICTED → MEMBER = unrestrict ❌ skip
            # ADMINISTRATOR/OWNER → MEMBER = demotion ❌ skip
